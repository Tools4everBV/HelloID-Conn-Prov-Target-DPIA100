...
if ($account.mail -ne $p.contact.business.email) {
    ...
    if(-Not($dryRun -eq $True)) {
        ...
        Try{
            ...
            $success = $True
            $auditMessage = "for person " + $p.DisplayName + ": DPIA100 export successfully written to $outFile"
        }
        Catch{
            $auditMessage = "for person " + $p.DisplayName + ": DPIA100 export failed to write $outFile $_"
        }
    } else {
